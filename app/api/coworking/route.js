...
export async function GET() {
  try {
    ...
    // Obtener todas las notas de coworking places en una sola query
    const placeIds = data.map(place => place.id);
    const { data: notesData } = await supabaseRead
      .from('notes')
      .select('*')
      .eq('related_type', 'coworking')
    ...
    console.error('Error fetching coworking places:', error);
...
export async function POST(request) {
  try {
    ...
    console.error('Error creating coworking place:', error);
    return Response.json({ error: error.message }, { status: 500 });
  }
}
